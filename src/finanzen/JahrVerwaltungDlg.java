 ...
 * @author fschuett
 * 
 * Die Verwaltung wird  in Schuljahren gerechnet.
 * 
 */
public class JahrVerwaltungDlg extends javax.swing.JDialog {
	...
	private void initComponents() {
		...
		jahrQuery = java.beans.Beans.isDesignTime() ? null : em
				.createQuery("SELECT j FROM Jahr j ORDER BY j.jahr"); // NOI18N
		...
		setTitle("Schuljahre verwalten");

		jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(null,
				"Schuljahre", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
		...
		entfernen
				.setToolTipText("Die Entfernung eines Jahres entfernt auch alle Finanzbewegungen für dieses Jahr. Alle Finanzbewegungen müssen 10 Jahre aufbewahrt werden.");
	...
	private void neuActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_neuActionPerformed
		String s = (String) JOptionPane.showInputDialog(this.getParent(),
				"Bitte geben Sie das gewünschte Schuljahr ein.",
				SchliessfachApp.getApplication().heute.get(Calendar.YEAR));
		try {
			if (s == null)
				return;
			Long l = Long.parseLong(s);
			Query q = em.createQuery("SELECT j.jahr FROM Jahr j WHERE j.jahr="
					+ l);
			if (!q.getResultList().isEmpty()) {
				JOptionPane.showMessageDialog(this.getParent(), "Das Jahr " + l
						+ " existiert bereits.", "Jahr erfassen",
			...
			em.persist(j);
			Historie.anhaengen(Rubrik.JAHR, j.getJahr().toString(),
					"hinzugefügt: " + j.toString());
			q = em.createQuery("SELECT v FROM Vertrag v WHERE v.beginnJahr <= "
					+ j.getJahr() + " AND v.endeJahr IS NULL OR " + j.getJahr()
					+ " BETWEEN v.beginnJahr AND v.endeJahr");
			@SuppressWarnings("unchecked")
			Iterator<Vertrag> vertragIt = q.getResultList().iterator();
			while (vertragIt.hasNext()) {
				Vertrag v = vertragIt.next();
				if (v.getEndeJahr() == null) {
					if (j.getJahr() > v.getBeginnJahr() || !v.isBeginnMitte()) {
						betrag = -SchliessfachApp.getApplication().aktuelleGebuehren
								.get(Zahlungsart.Miete);
						z = new Zahlung(v, Zahlungsart.Miete, j.getJahr()
								.intValue(), betrag);
					} else {
						betrag = -SchliessfachApp.getApplication().aktuelleGebuehren
								.get(Zahlungsart.MieteHalbjahr);
						z = new Zahlung(v, Zahlungsart.MieteHalbjahr, j
								.getJahr().intValue(), betrag);
					}
				} else {
					/* Jahr == endeJahr && Vertrag beendet */
					if (v.getBeginnJahr() < v.getEndeJahr()) {
						if (v.isEndeMitte()) {
							betrag = -SchliessfachApp.getApplication().aktuelleGebuehren
									.get(Zahlungsart.MieteHalbjahr);
							z = new Zahlung(v, Zahlungsart.MieteHalbjahr, j.getJahr()
									.intValue(), betrag);
						} else {
							betrag = -SchliessfachApp.getApplication().aktuelleGebuehren
									.get(Zahlungsart.Miete);
						...
						if(v.isBeginnMitte() && v.isEndeMitte())
							continue;
						else {
							betrag = -SchliessfachApp.getApplication().aktuelleGebuehren
									.get(Zahlungsart.MieteHalbjahr);
	...
	private void entfernenActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_entfernenActionPerformed
		if(jahr.getSelectedValue() == null)
			return;
		int j = ((Jahr) jahr.getSelectedValue()).getJahr().intValue();
		if (j == -1) {
			JOptionPane.showMessageDialog(this.getParent(),
					"Sie müssen zunächst ein Jahr in der Liste wählen.",
					"Jahr entfernen", JOptionPane.ERROR_MESSAGE);
			return;
		}
		Query q = em.createQuery("DELETE FROM Jahr j WHERE j.jahr=" + j);
		em.getTransaction().begin();
		q.executeUpdate();
		q = em.createQuery("DELETE FROM Zahlung z WHERE "
				+ Zahlungsart.mietenQS("z.leihJahr=" + j + " AND z.art"));
		q.executeUpdate();
		em.getTransaction().commit();
	...
	public static void main(String args[]) {
		java.awt.EventQueue.invokeLater(new Runnable() {
			public void run() {
				JahrVerwaltungDlg dialog = new JahrVerwaltungDlg(
